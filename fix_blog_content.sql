...
SELECT title, slug, LENGTH(content) as content_length, content FROM public.blogs 
WHERE slug = 'vegetarian-restaurant-brussels-east-at-west';

-- Delete the existing truncated blog post
DELETE FROM public.blogs WHERE slug = 'vegetarian-restaurant-brussels-east-at-west';

-- Insert the complete blog post with proper content
...
Whether you are enjoying a fine dining experience or grabbing a quick lunch, the focus is always on clean eating and nutrition.
...
This Act was inspired by Upton Sinclair novel The Jungle, which exposed the unsanitary conditions of the meatpacking industry.
...
What is on the Menu at a Vegetarian Restaurant?
...
Belgium Must-Try Vegetarian Dishes

When you visit East @ West Vegetarian Restaurant in Brussels, do not miss these signature dishes:
...
What is the Cost of Opening a Vegetarian Restaurant?
...
There is growing demand for creative dishes that challenge stereotypes about vegetarian cuisine.
...
-- Verify the new blog post
SELECT title, slug, LENGTH(content) as content_length, SUBSTRING(content, 1, 200) as content_preview FROM public.blogs 
WHERE slug = 'vegetarian-restaurant-brussels-east-at-west'; 